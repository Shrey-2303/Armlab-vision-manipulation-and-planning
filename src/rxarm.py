...
class RXArm(InterbotixManipulatorXS):
    ...
    def set_positions(self, joint_positions):
        ...
        # difference = max(abs(a-b) for a, b in zip(current_positions, joint_positions))
        # scaling_factor=0.1
        # moving_time = 1.0 + scaling_factor * difference
        # accel_time = 0.3 + 0.5 * scaling_factor * difference
        # self.set_moving_time(moving_time)
        # self.set_accel_time(accel_time)
    ...
    def disable_gripper_torque(self):
        """!
        @brief      Disables only the gripper torque.
        """
        self.core.robot_set_motor_registers('single', 'gripper', 'Torque_Disable', 1)
        
    def enable_torque(self):
        """!
        @brief      Enables the torque and estops.
        """
        self.core.robot_set_motor_registers('group', 'all', 'Torque_Enable', 1)
    
    def enable_gripper_torque(self):
        """!
        @brief      Enables only the gripper torque.
        """
        self.core.robot_set_motor_registers('single', 'gripper', 'Torque_Enable', 1)
...
#   @_ensure_initialized
    ...
    def get_ee_pose(self):
        ...
        """
        t1, t2, t3, t4, t5 = list(self.get_positions())

        t1 = t1 - self.poses_at_init[0]
        t2 = t2 - self.poses_at_init[1]
        t3 = t3 - self.poses_at_init[2]
        t4 = t4 - self.poses_at_init[3]
        t5 = t5 - self.poses_at_init[4]
        
        dh_matrix = np.array([
                                [t1+np.pi/2, 103.91, 0, -np.pi/2],
                                #[t2+np.pi/2, 0, -200, 0],
                                #[-np.pi/2, 0, 50, 0]
                                [t2-1.3258176637, 0, 205.73, 0],
                                [t3+1.3258176637, 0, 200, 0],
                                [t4-np.pi/2, 0, 0, -1*np.pi/2],
                                [t5+np.pi, 152.575, 0, 0]
                            ])
        T_matrix = FK_dh(dh_matrix,5)
        pose = get_pose_from_T(T_matrix)
        return pose
